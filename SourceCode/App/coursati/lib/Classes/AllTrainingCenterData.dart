...
AllTrainingCenterData({required this.tcData,required this.courses,required this.trainers});
...
factory AllTrainingCenterData.fromJson(Map<String,dynamic> json){
  ...
  List<BoxCourseLabelData> _courses =
   courseObjJson.map((courseJson)=>BoxCourseLabelData.fromJson(courseJson)).toList();
  var trainerObjJson = json["trainers"]as List;
  List<Trainer> _trainers = trainerObjJson.map((trainerJson)=>Trainer.fromJson(trainerJson)).toList();

  return AllTrainingCenterData(
    tcData: TrainingCenter.fromJson(json["TrainingCenter"]),
    courses: _courses,
    trainers: _trainers);
